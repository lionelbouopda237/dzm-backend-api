...
async function buildEmballagesSummary() {
  const [{ data: factures }, { data: mouvements }] = await Promise.all([
  ...
  for (const structure of ['DZM A', 'DZM B']) {
    base.set(structure, { structure, emballagesPleins: 0, emballagesVides: 0, colis: 0, dette: 0, valeurDeduction: 0 });
  }
  for (const row of (factures || [])) {
    const target = base.get(detectStructure(row.structure));
    target.emballagesPleins += Number(row.nombre_casiers || 0);
    target.emballagesVides += Number(row.casiers_retournes || 0);
  }
  for (const row of (mouvements || [])) {
    const target = base.get(detectStructure(row.structure));
    target.emballagesPleins += Number(row.emballages_pleins || 0);
    target.emballagesVides += Number(row.emballages_vides || 0);
    target.colis += Number(row.colis || 0);
  }
  const result = Array.from(base.values()).map((item) => ({ ...item, dette: item.emballagesPleins - item.emballagesVides, valeurDeduction: item.emballagesVides * 3600 }));
  return { summary: result, mouvements: mouvements || [] };
}
...
app.get('/health', (_, res) => res.json({ status: 'ok', service: 'DZM Backend v4 - Groq + Supabase + Cloudinary', timestamp: new Date().toISOString() }));
...
app.listen(PORT, () => console.log(`✅ DZM Backend v4 - http://localhost:${PORT}`));
